...
densities given the observed data and selected priors.

<!-- Examples of those plots from this analysis are provided in Figure 1, but for  -->
<!-- brevity's sake not all examined plots are included. From Table 1 and Figures ... -->
<!-- we see that ... -->
...
Appendix in Tables 2 and 3, respectively.
...
associated with 1.091 times higher HIV copies in a mL of blood. For the 
...
For the multivariate model, the same coefficient is estimated to be -0.166
(95% HPD: -0.566, 0.227), which implies that drug use is, on average,
associated with 0.682 times lower HIV copies in an mL of blood following two 
years of HAART. For this model, the DIC was estimated to be 1410.898, while the
same model without drug use as a predictor had a DIC of 1410.525. The
posterior probability of the drug use coefficient corresponding to a 10% change
was 0.875.

### CD4+ Cells

For the univariate model with CD4+ cell count as the outcome, 
the drug use coefficient for drug use was estimated
to be -170.989 (95% HPD: -232.818, -112.506), suggesting that on average
drug users have -170.989 lower CD4+ cells than non-drug users after 2 years of
...
the coefficient for drug use was estimated to be -0.135, but the 95% HPD
(-3.526, 3.250) is practically centered at 0. This, coupled with the low
posterior probability of a 10% change in mental quality of life score from
drug use (0.008) and the slight increase in DIC when including drug use as a coefficient,
are all indicative that the data does not support an association between drug
use and a change in mental quality of life score after two years of HAART.
...
For the univariate model wiht physical quality of life score as the outcome,
...
in physical quality of life score of 0.221. The DIC with the inclusion of drug
...
change drops to 0.045, both indicating some level of confounding between the
additional variables and drug uses relationship with physical quality of life.

The results are mostly consistent with those from the frequentist analysis,
however in the frequentist the p-values suggest weak evidence that drug use
and physical quality of life score may be correlated. As such, there does
appear to be some association, but it is highly unlikely to correspond
with a 10% change in average physical quality of life score between users and
non-users 2 years after starting HAART.
...
indicative that a meaningful correlation may exist. Given this consideration
coupled with this study's results, further research that aims to expand the
...
files: `1_prepData.R`, `2_-_EDA.R`, `3_FrequentistAnalysis.R`, 
...
kable(fullBayesianSummary[, 2:7], booktabs=TRUE, 
      ...
      digits = 3, col.names = c("Model", "Penalized DIC", "Estimate", 
                                "HPD Lower", "HPD Upper", 
...
kable(fullFreqSummary[, 2:7], booktabs=TRUE, 
      ...
      digits = 3, col.names = c("Model", "AIC", "Estimate", "Lower CI", 
                                "Upper CI", "p-value")) %>%
    kable_styling(latex_options = c("striped", "scale_down")) %>% 
    add_header_above(c(" " = 2, "Drug Use Coefficient" = 4)) %>%
    pack_rows("Standardized Viral Load (log10 copies/ml)", 1, 4) %>%
