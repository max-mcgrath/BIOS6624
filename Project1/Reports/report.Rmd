...
that were deemed implausible were also removed.
...
age, body mass index, smoking status, education, race/ethnicity. All confounding 
values were those measured at baseline, except adherence which used second year
values. Those models will be reffered to as multivariable models. Lastly, for each
...
predictor. We will refer to those models as univariable without drug use and
...
rate parameters both equal to 0.001.
...
```{r, echo = FALSE}
kable(fullBayesianSummary, booktabs=TRUE, caption = "Bayesian Summary", 
      digits = 2) %>%
...
```{r, echo = FALSE}
kable(fullFreqSummary, booktabs=TRUE, caption = "Frequentist Summary",
      digits = 2) %>%
