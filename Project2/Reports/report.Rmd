...
and changes
in episodic memory and cortical thickness in one year as outcomes and changes
in cytokines and chemokines in one year as predictors for the second part. 

We have also been given a large number of potentially confounding factors such
as age, sex, inflammation, cardiovascular risk, health history, and APOE 
...
As with Aim 1, we will use elastic net regression to identify a set of
...
subjects, so more enrollment is necessary.
...
throughout the duration of the grant. It
will also fund a data manager who will primarily be tasked with the creation
of a REDCap data set and data management in the first two years of the grant,
...
$37,490 for the third,
$92,860 for the fourth,
...
yearTwoCost <- c(salaryWithBenefits[1]*yearTwoEffort[1], 
                 ...
                 salaryWithBenefits[3]*yearTwoEffort[3] + 
                 salaryWithBenefits[4]*yearTwoEffort[4])
yearThreeEffort <- c(.05, .15, .1, .15, NA)
