...
# Read data
nSim <- readRDS("DataRaw/nSim2b.rda")
coefEstsBS <- readRDS("DataRaw/coefEstsBS2b.rda")
coefEstsAIC <- readRDS("DataRaw/coefEstsAIC2b.rda")
coefEstsBIC <- readRDS("DataRaw/coefEstsBIC2b.rda")
coefEstsLASSOCV <- readRDS("DataRaw/coefEstsLASSOCV2b.rda")
coefEstsLASSOFIX <- readRDS("DataRaw/coefEstsLASSOFIX2b.rda")
coefEstsENCV <- readRDS("DataRaw/coefEstsENCV2b.rda")
coefEstsENFIX <- readRDS("DataRaw/coefEstsENFIX2b.rda")

# Model summary table ----------------------------------------------------------
modelDF2b <- data.frame(
...
modelDF2b <- cbind("stat" = rownames(modelDF2b), 
                   data.frame(modelDF2b, row.names = NULL))

modelDF2b$stat <- c("Total TPR", "FPR", "FDR", "Type I Error", "Type 2 Error")
...
coefDF2b <- cbind("stat" = rownames(coefDF2b), 
                  data.frame(coefDF2b, row.names = NULL))

coefDF2b$stat <- c("Bias", "95\\% CI", "TPR",
                   "Bias", "95\\% CI", "TPR",
                   "Bias", "95\\% CI", "TPR",
                   "Bias", "95\\% CI", "TPR",
                   "Bias", "95\\% CI", "TPR")

colnames(modelDF2b) <- colnames(coefDF2b)
