...
first, predictors were removed one at a time according to highest p-value
until all predictors have a p-value under $0.15$. For the second and third,
we used AIC and BIC to iteratively reduce the model until
...
For regularization, we applied LASSO and Elastic Net, each with both
...
coefficient estimate is greater than zero removed if their coefficient estimate
is equal to zero. For full details on LASSO and Elastic Net see Tibsheranti (1996) and
Zou and Hastie (2004). For LASSO and Elastic Net, the `cv.glment()` and `glmnet()` functions
...
induce exchangeable correlation. The response for each observation is then drawn from a normal
...
equal 1 if predictor $X_p,p=1,\ldots,20$ was retained for simulation $i=1,2,\ldots,1000$ and 0 
...
Figure 1 provides plots of several of the above model selection technique 
evaluation criteria for each case scenario, and Tables 1-4 provide more detailed
results of the evaluation criteria for cases 1a, 1b, 2a, and 2b, respectively.

From Figure 1 and the tables, we see that for TPR, backwards selection using p-values and
AIC is roughly the same, but is lower when using BIC for backwards selection.
Further, LASSO with CV and ENet with CV both have the best overall TPR of the
seven methods. Those trends are reversed for FPR and FDR, where we see that
BIC outperforms p-value and AIC selection, and the regularization methods with
CV performing worse than the backwards selection techniques. 

For TPR, we see that backwards selection generally performs slightly worse (lower) 
than regularization methods across the four case scenarios, with BIC performing
the worst among the seven techniques in all scenarios (disregarding regularization with
fixed parameters as we will often do for the remainder of this
analysis - their behavior is weird and preselected regularization parameters are
unlikely to be used in practice). This trend is reversed for FPR where
backwards selection generally outperforms (has lower FPR) 

For Type I error, we see fairly consistent behavior across methods, with 
backwards selection using p-values and AIC generally performing the best,
...
For coefficient bias, we see that backward selection techniques greatly 
outperform regularization methods. This is to be expected, as regularization
methods induce bias in their coefficient estimates in order to reduce model
variance.
...
<!-- DISCUSS BIAS BUT MAYBE FIX YOUR SIMS FIRST -->

## Discussion

- We see strong consistency between false positive rate and false discovery rate
across both different case scenarios and model selection techniques. 
...
that across the four case scenarios the change in TTPR between AIC and BIC and
that of FPR is notably different. For BIC, all scenarios had a fairly
consistent FPR (0.015 - 0.024) while for TTPR we see a greater range (0.880 - 
0.980). This is noteworthy because it is indicative that the trade-off may be
more beneficial for BIC in certain scenarios. Specifically, BIC had the lowest
TTPR for 1b where data was limited as predictors correlated, whereas it was
higher as data increased and predictors became less correlated. This is similar
to the trend we see with other model selection techniques

- Performance between AIC and pVal is very similar across case scenarios, with
the exception of 2a.

- TTPR essentially measures the TPR for coefficient one, as that was the only
coefficient with a TPR consistently below 1
...
- Given the incredibly high true positive rates for the majority of predictors,
the results of this analysis may differ for the detection of smaller effects.

- The correlation imposed on the predictors was simple and likely unrealistic.
A more comprehensive analysis would consider imposing more complex correlation
structures on the predictors. 

- LASSO and ENet with pre-specified regularization parameters is detached from
